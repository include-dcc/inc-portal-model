...
class Variable:
    header = ["varname", "vardesc", "type", "values", "min", "max"]
    ...
    def writerow(self, writer, max_categorical_count):
        ...
        if floaters > inters and floaters > others:
            ...
            if len(self.distinct_values) <= max_categorical_count:
                categoricals = ";".join(sorted(list(self.distinct_values)))
            writer.writerow([self.varname, self.varname, "String", categoricals, "", ""])

if __name__=="__main__":
    parser = ArgumentParser(description="After looking through the file, this script will attempt to create a data dictionary")
